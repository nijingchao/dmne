...
function [ecs, dcs, Us, Hs, objvals, ft_t] = dmne_ed(G_mats, O_mats, S_nm_mats, alpha, beta, lambda, ...
...
objval = dmneobj_ed(G_mats, O_mats, S_nm_mats, Us, weight_vec, ecs, dcs, ns, alpha, beta, lambda);
...
while iter <= maxepoch
    ...
    while iter2 <= maxepoch2
        ...
        for i = 1:g
            U_i = update_u_ed(O_mats(i,:), O_mats(:,i), S_nm_mats(i,:), S_nm_mats(:,i), Us, Hs{i}, ns, i, alpha, beta);
            Us{i} = U_i;
        end
